...
class Ion_auth_model extends CI_Model
{
	 ...
	 * @author Ben Edmunds
	 **/
	public function modules_menu($parent = NULL, $modules = NULL)
	{
            $is_parent = ($parent == 'parent' ? '1' : '');
            
//            $grup       = $this->ion_auth->get_users_groups()->row();
//            $grup_jn    = $this->ion_auth->group($grup->id)->row();
//            $akses      = json_decode($grup->akses);
...
//            if(!empty($parent)){
//		$tb  = $this->db->select('id, modules, modules_action, modules_name, modules_route, sort')
//                                ->order_by($this->tables['modules'].'.sort', 'asc')
//                                ->where($this->tables['modules'].'.is_parent', $is_parent)
//                                ->get($this->tables['modules'])->result();                
//            }else{
//		$tb  = $this->db->select('id, modules, modules_action, modules_name, sort')
//                                ->order_by($this->tables['modules'].'.sort', 'asc')
//                                ->get($this->tables['modules'])->result();
//                
//            }
