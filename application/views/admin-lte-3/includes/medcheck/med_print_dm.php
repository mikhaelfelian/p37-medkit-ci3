...
<body>
    <table border="0" style="width: 365px;" cellspacing="0">
        <tr>
            <th rowspan="3" style="border-bottom: 1px solid #000;">
                <img src="<?php echo base_url('/assets/theme/admin-lte-3/dist/img/kop_es_bw_197x234.png') ?>"
        ...
        <tr>
            <td style="width: 75px; text-align: center; font-size: 11px; border-bottom: 1px solid #000; color: #00A650;"
                colspan="5">
        ...
        <tr>
            ...
            <td style="font-size: 11px;">
                <?php echo $this->tanggalan->tgl_indo2($sql_medc->tgl_masuk) ?><br /><?php echo $this->tanggalan->wkt_indo($sql_medc->tgl_masuk) ?>
                - <?php echo $this->tanggalan->wkt_indo($sql_medc->tgl_bayar) ?>
            </td>
        </tr>
        <tr>
        ...
        <tr>
            ...
            <th style="font-size: 11px;">:</th>
            <td style="font-size: 11px;">
                <?php echo (!empty($sql_medc->id_kasir) ? $this->ion_auth->user($sql_medc->id_kasir)->row()->first_name : $this->ion_auth->user()->row()->first_name); ?>
            </td>
        </tr>
    </table>
    <!--=======================================================================================================-->
    <table border="0" style="width: 365px;" cellspacing="0">
        <tr>
            <th style="text-align: left; border-top: 1px dashed #000; border-bottom: 1px dashed #000; font-size: 11px;">
                Item</th>
            <th
                style="text-align: center; border-top: 1px dashed #000; border-bottom: 1px dashed #000; font-size: 11px;">
                Jml</th>
            <th
                style="text-align: center; border-top: 1px dashed #000; border-bottom: 1px dashed #000; font-size: 11px; width: 5px;">
            </th>
            <th
                style="text-align: right; border-top: 1px dashed #000; border-bottom: 1px dashed #000; width: 100px; font-size: 11px; width: 50px;">
                Harga</th>
            <th
                style="text-align: center; border-top: 1px dashed #000; border-bottom: 1px dashed #000; font-size: 11px; width: 5px;">
            </th>
        ...
        <?php foreach ($sql_medc_det as $det) { ?>
            ...
            <tr>
                ...
                <th style="text-align: left; font-size: 11px;" colspan="6">
                    <?php echo $sql_kat->keterangan . ' (' . $sql_kat->kategori . ')'; ?>
                </th>
            </tr>
            <?php $subtotal = 0; ?>
            <?php foreach ($sql_det as $medc) { ?>
                ...
                <tr>
                    <td style="text-align: left; font-size: 11px;">
                        <?php echo ($medc->status_rc == '1' ? ' -<i>' . $medc->item . '</i>' : $medc->item); ?>
                    </td>
                ...
                <tr>
                    <td style="text-align: right; font-size: 11px; font-style: italic;" colspan="2">
                        <?php echo ($medc->disk1 != '0.00' ? 'disk : ' . (float) $medc->disk1 : '') . ($medc->disk2 != '0.00' ? ' + ' . (float) $medc->disk2 : '') . ($medc->disk3 != '0.00' ? ' + ' . (float) $medc->disk3 : '') . ($medc->disk1 != '0.00' || $medc->disk2 != '0.00' || $medc->disk3 != '0.00' ? '%' : '') . ($medc->potongan > 0 ? ' pot : ' . general::format_angka($medc->potongan) : ''); ?>
                    </td>
                    <td style="text-align: center; font-size: 11px; font-style: italic;"></td>
                    <td style="text-align: right; font-size: 11px; font-style: italic;">
                        <?php echo (!empty($total_disk) ? '(' . general::format_angka($total_disk) . ')' : ''); ?>
                    </td>
                    <td style="text-align: center; font-size: 11px; font-style: italic;"></td>
                ...
                <?php foreach (json_decode($medc->resep) as $racikan) { ?>
                    <tr>
                        ...
                        <td style="text-align: center; font-size: 11px;">Rp.</td>
                        <td style="text-align: right; font-size: 11px;"><?php echo general::format_angka($racikan->subtotal); ?>
                        </td>
                    </tr>
                <?php } ?>
            ...
            <tr>
                ...
                <td style="text-align: right; font-weight: bold; font-size: 11px;">
                    <?php echo general::format_angka($subtotal); ?></td>
            </tr>
            <?php $gtotal = $gtotal + $subtotal; ?>
            <?php $number++; ?>
        <?php } ?>
        <?php
        // Get payment platform information
        $sql_platform = $this->db->where('id', $sql_medc->metode)
                                ->get('tbl_m_platform')
                                ->row();

        // Calculate financial values
        $jml_ongkir   = $sql_medc->jml_ongkir;
        $jml_total    = $sql_medc_sum->subtotal + 
                        $sql_medc_sum->diskon + 
                        $sql_medc_sum->potongan + 
                        $sql_medc_sum->potongan_poin;
        ...
        ?>
        <tr>
            <td colspan="2" style="text-align: left; font-weight: bold; font-size: 9px; border-top: 1px dashed #000;">
                <?php echo $sql_platform->platform ?>
            </td>
            <td colspan="2" style="text-align: right; font-weight: bold; font-size: 11px; border-top: 1px dashed #000;">
                Grand Total</td>
            <td style="text-align: center; font-size: 11px; font-weight: bold; border-top: 1px dashed #000;">Rp.</td>
            <td style="text-align: right; font-weight: bold; font-size: 11px; border-top: 1px dashed #000;">
                <?php echo general::format_angka($jml_total); ?>
            </td>
        </tr>
        <tr>
            <td colspan="2" style="text-align: left; font-weight: bold; font-size: 9px;">PARAF</td>
            <td colspan="2" style="text-align: right; font-weight: bold; font-size: 11px;">Disc
                (<?php echo (float) round($diskon, 1) ?>%)</td>
            <td style="text-align: center; font-size: 11px; font-weight: bold;">Rp.</td>
            <td style="text-align: right; font-weight: bold; font-size: 11px;">
                -<?php echo general::format_angka($jml_diskon) ?></td>
        </tr>
        <tr>
            <td colspan="2" style="text-align: left; font-weight: bold; font-size: 9px;">Poin :
                <?php echo ($sql_pasien_poin->jml_poin <= 0 ? 0 : (float) $sql_pasien_poin->jml_poin) ?>
            </td>
            <td colspan="2" style="text-align: right; font-weight: bold; font-size: 11px;">Ongkir</td>
            <td style="text-align: center; font-size: 11px; font-weight: bold;">Rp.</td>
            <td style="text-align: right; font-weight: bold; font-size: 11px;">
                <?php echo general::format_angka($jml_ongkir) ?>
            </td>
        </tr>
        <tr>
            <td rowspan="1" style="text-align: left; font-weight: bold; font-size: 11px;">
                <?php if (!empty($sql_medc->ttd_obat)) { ?>
                    <!--<img src="<?php // echo base_url($sql_medc->ttd_obat) ?>" style="width: 50px;">-->
                <?php } ?>
            </td>
            <td colspan="3" style="text-align: right; vertical-align: top; font-weight: bold; font-size: 11px;">Harus
                Dibayar</td>
            <td style="text-align: center; vertical-align: top; font-size: 11px; font-weight: bold;">Rp.</td>
            <td style="text-align: right; vertical-align: top; font-weight: bold; font-size: 11px;">
                <?php echo general::format_angka($jml_subtotal) ?>
            </td>
        </tr>
        <?php $jml_tot_byr = 0; ?>
        <?php foreach ($sql_medc_plat as $plat) { ?>
            ...
            <tr>
                <td colspan="4" style="text-align: right; font-weight: bold; font-size: 11px;">
                    <?php echo $sql_plat->platform ?>
                </td>
                <td style="text-align: center; font-size: 11px; font-weight: bold;">Rp.</td>
                <td style="text-align: right; font-weight: bold; font-size: 11px;">
                    <?php echo general::format_angka($plat->nominal) ?>
                </td>
            </tr>
        <?php } ?>
        ...
        <tr>
            ...
            <td style="text-align: center; font-size: 11px; font-weight: bold;">Rp.</td>
            <td style="text-align: right; font-weight: bold; font-size: 11px;">
                <?php echo '-' . general::format_angka($jml_kembali) ?></td>
        </tr>
        <tr>
            <td colspan="6" style="text-align: center; border-top: 1px double #000; font-size: 11px;">Terimakasih atas
                kunjungannya, semoga lekas sembuh</td>
        </tr>
        <tr>
            <td colspan="6" style="text-align: center; border-bottom: 1px double #000; font-size: 15px;">Transaksi yang
                sudah dibayar tidak dapat dibatalkan</td>
        </tr>
    </table>
    <br />
