    ...
    <div class="content">
        <div class="container-fluid">
            <div class="row">
                <div class="col-lg-8">
                    ...
                    <div class="card">
                                                        ...
                                                        <tr>
                                                            ...
                                                            <td class="text-right" style="width: 150px;">
                                                                ...
                                                                <?php if ($cart['options']['disk'] != '0') { ?>
                                                                    ...
                                                                    <?php echo br(); ?>
                                                                    <small>(<?php echo (float) $cart['options']['disk1'] . ($cart['options']['disk2'] != '0' ? ' + ' . $cart['options']['disk2'] : '') . ($cart['options']['disk3'] != '0' ? ' + ' . $cart['options']['disk3'] : '') ?>
                                                                        %)</small>
                                                                <?php } ?>
                                                            </td>
                                                            ...
                                                            <td class="text-right">
                                                                <?php echo anchor(base_url('pos/lab/set_trans_jual_hapus_item.php?id=' . $this->input->get('id') . '&item_id=' . general::enkrip($cart['rowid'])), '<i class="fa fa-trash"></i> Hapus', 'class="btn btn-danger btn-flat btn-xs" style="width: 55px;" onclick="return confirm(\'Hapus [' . $cart['name'] . '] ?\')"') ?>
                                                            </td>
                                                        </tr>
                                                    ...
                                                    <?php if (!empty($x)) { ?>
                                                        <tr>
                                                            ...
                                                            <td class="text-right text-bold">
                                                                <?php echo general::format_angka($this->cart->total()); ?>
                                                            </td>
                        ...
                        <div class="card-footer">
                            <?php if (!empty($x)) { ?>
                                <?php echo form_open(base_url('pos/lab/set_trans_jual_proses.php'), 'id="apt_proses_form" autocomplete="off"'); ?>
                                <?php echo add_form_protection() ?>
                                <?php echo form_hidden('id', $this->input->get('id')); ?>

                                <button type="submit" class="btn btn-info float-right rounded-0"
                                    onclick="return confirm('posting ?')"><i class="fa fa-arrows-rotate"></i>
                                    Posting</button>
                                <?php echo add_double_submit_protection('apt_proses_form') ?>
                                <?php echo form_close(); ?>
                            <?php } ?>
                        </div>
                    </div>
                ...
                <div class="col-lg-4">
                    <?php echo form_open(base_url('pos/lab/set_trans_jual_upd.php'), 'id="apt_form" autocomplete="off"') ?>
                    <?php echo add_form_protection() ?>
                    <div class="card">
                        <div class="card-header">
                        ...
                        <div class="card-body">
                            ...
                            <div class="form-group">
                                ...
                                <div class="input-group mb-3">
                                    ...
                                    <div class="input-group-append">
                                        <span
                                            class="input-group-text rounded-0"><?php echo anchor(base_url('pos/lab/data_pelanggan_tambah.php'), '<i class="fas fa-plus text-bold"></i>', 'class="text-default"') ?></span>
                                    </div>
                                </div>
                            </div>
                            <div class="form-group">
                        ...
                        <div class="card-footer">
                            <a href="<?php echo base_url('pos/lab/set_trans_jual_batal.php?id=' . $this->input->get('id')) ?>"
                                class="btn btn-danger rounded-0" onclick="return confirm('Batalkan transaksi ?')"><i
                                    class="fa fa-refresh"></i> Batal</a>
                            <button type="submit" class="btn btn-info float-right rounded-0"><i class="fa fa-save"></i>
                                Simpan</button>
                        </div>
                    </div>
                    <?php echo add_double_submit_protection('apt_form') ?>
                    <?php echo form_close(); ?>
                    <?php echo form_open(base_url('pos/lab/set_trans_jual_simpan_item.php'), 'id="apt_item_form" autocomplete="off"') ?>
                    <?php echo add_form_protection() ?>
                    <div class="card">
                        <div class="card-header">
                        ...
                        <?php if (!empty($sql_produk)) { ?>
                            <div class="card-footer">
                                <button type="button" class="btn btn-danger rounded-0"
                                    onclick="window.location.href = '<?php echo base_url('medcheck/invoice/bayar.php?id=' . $this->input->get('id')) ?>'"><i
                    ...
                    <?php echo add_double_submit_protection('apt_item_form') ?>
                    <?php echo form_close(); ?>
                </div>
            </div>
        </div>
    ...
    $(function () {
        ...
        $('#pelanggan').autocomplete({
            source: function (request, response) {
                $.ajax({
                    url: "<?php echo base_url('pos/lab/json_customer.php') ?>",
                    dataType: "json",
                    data: {
                        term: request.term
                    },
        ...
        $('#produk').select2({
            ...
            ajax: {
                url: "<?php echo base_url('pos/lab/json_item.php') ?>",
                dataType: 'json',
                delay: 250,
            ...
            window.location.href = "<?php echo base_url('pos/lab/trans_jual.php?id=' . $this->input->get('id')) ?>&item_id=" + data.id + "&harga=" + data.harga + "&satuan=" + data.satuan;
