                ...
                <div class="col-md-12">
                    <div class="card card-default">
                        ...
                        <div class="card-body table-responsive">
                            <div class="row">
                                <div class="col-md-6">
                                    <?php echo form_open(base_url('transaksi/beli/set_trans_beli_po.php'), 'id="po_form" autocomplete="off"') ?>
                                    <?php echo add_form_protection() ?>
                                    <input type="hidden" id="id_supplier" name="id_supplier">                                  

                                    <div class="form-group <?php echo (!empty($hasError['id_supplier']) ? 'text-danger' : '') ?>">
                                        <label class="control-label">Nama Supplier*</label>
                                        <?php echo form_input(array('id' => 'supplier', 'name' => 'supplier', 'class' => 'form-control rounded-0 text-middle' . (!empty($hasError['id_supplier']) ? ' is-invalid' : ''), 'style' => 'vertical-align: middle;', 'placeholder' => 'Isikan Nama Supplier ...')) ?>
                                    </div>
                                    <div class="row">
                                        <div class="col-md-12">
                                            <div class="form-group <?php echo (!empty($hasError['tgl_beli']) ? 'text-danger' : '') ?>">
                                                ...
                                                <div class="input-group mb-3">
                                                    ...
                                                    <?php echo form_input(array('id' => 'tgl', 'name' => 'tgl_masuk', 'class' => 'form-control rounded-0 pull-right' . (!empty($hasError['tgl_masuk']) ? ' is-invalid' : ''), 'placeholder' => 'Isikan Tgl PO ...')) ?>
                                                </div>
                                            </div>                                            
                                        </div>
                                    </div>
                                    <div class="form-group">
                                        ...
                                        <div class="input-group mb-3">
                                            <?php echo form_input(array('id' => 'keterangan', 'name' => 'keterangan', 'class' => 'form-control rounded-0 text-middle' . (!empty($hasError['pasien']) ? ' is-invalid' : ''), 'style' => 'vertical-align: middle;', 'placeholder' => 'Isikan Keterangan ...')) ?>
                                        </div>
                                    </div>
                                    <div class="form-group">
                                        <label class="control-label">Alamat Pengiriman</label>
                                        <div class="input-group mb-3">
                                            <?php echo form_textarea(array('id' => 'pengiriman', 'name' => 'pengiriman', 'class' => 'form-control rounded-0 text-middle' . (!empty($hasError['pasien']) ? ' is-invalid' : ''), 'style' => 'vertical-align: middle;', 'placeholder' => 'Isikan Alamat ...')) ?>
                                        </div>
                                    </div>
                                    <div class="row">
                                        <div class="col-md-6">
                                    ...
                                    <?php echo add_double_submit_protection('po_form') ?>
                                    <?php echo form_close() ?>
                                </div>
                                <div class="col-md-6">
                                    <?php if (!empty($sess_beli)) { ?>
                                        <?php echo form_open(base_url('transaksi/beli/cart_beli_po_simpan.php'), 'id="po_item_form" autocomplete="off"') ?>
                                        <?php echo add_form_protection() ?>
                                        <input type="hidden" id="no_nota" name="no_nota" value="<?php echo general::enkrip($sql_beli->id) ?>">                                  
                                        <input type="hidden" id="id_item" name="id_item" value="<?php echo general::enkrip($sql_item->id) ?>">                                  

                                        <div class="form-group <?php echo (!empty($hasError['id_item']) ? 'text-danger' : '') ?>">
                                            <label class="control-label">Kode *</label>
                                            <?php echo form_input(array('id' => 'kode', 'name' => 'kode', 'class' => 'form-control rounded-0 text-middle' . (!empty($hasError['id_item']) ? ' is-invalid' : ''), 'style' => 'vertical-align: middle;', 'placeholder' => 'Isikan Kode ...', 'value' => $sql_item->kode)) ?>
                                        </div>
                                        ...
                                        <?php echo add_double_submit_protection('po_item_form') ?>
                                        <?php echo form_close() ?>
                                    <?php } ?>
                                </div>
                            </div>
                                                    ...
                                                    <?php $sql_sat  = $this->db->where('id', $sql_brg->id_satuan)->get('tbl_m_satuan')->row() ?>
                                                        <tr>
                                                            <td class="text-right" style="width: 50px;">
                                                                <?php echo anchor(base_url('transaksi/beli/cart_beli_po_hapus.php?id='.general::enkrip($sql_beli->id).'&item_id='.general::enkrip($cart->id)), '<i class="fas fa-trash"></i>', 'class="btn btn-danger btn-sm rounded-0" onclick="return confirm(\'Hapus [' . $cart->produk . '] ?\')"') ?>
                                                            </td>
                                                            ...
                                                            <td class="text-right"><?php echo $cart->keterangan_itm; ?></td>
                                                        </tr>
                                                        <?php $no++; ?>
                                                    <?php } ?>
                                                <?php } else { ?>
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-md-6 text-right">
                                        <?php if (!empty($sql_beli)) { ?>
                                            <?php echo form_open(base_url('transaksi/beli/set_trans_beli_po_proses.php'), 'id="po_proses_form" autocomplete="off"') ?>
                                            ...
                                            <?php echo add_double_submit_protection('po_proses_form') ?>
                                            <?php echo form_close() ?>
                                        <?php } ?>
                                    </div>
                                </div>
...
<?php if (!empty($sess_beli)) { ?>
            ...
            $('#kode').autocomplete({
                ...
                select: function (event, ui) {
                    ...
                    window.location.href = "<?php echo base_url('transaksi/beli/trans_beli_po.php?id='.general::enkrip($sql_beli->id)) ?>&id_item=" + ui.item.id_item + "&harga=" + ui.item.harga_beli;
