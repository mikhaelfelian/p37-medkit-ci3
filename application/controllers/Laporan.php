                ...
                switch ($case){
                    case 'per_tanggal':
                        $data['sql_remun']       = $this->db
                                                        ...
                                                        ->like('status_produk', (!empty($tipe) ? $tipe : '2'))
        ...
        if (akses::aksesLogin() == TRUE) {
            ...
            if($jml > 0){
                ...
                switch ($case){
                    case 'per_tanggal':
                        if(!empty($hal)){
                            $data['sql_omset']     = $this->db->select('id, id_medcheck, id_poli, tipe, tgl_simpan, tgl_bayar, no_rm, pasien, tgl_lahir, tipe, tipe_bayar, SUM(diskon) AS jml_diskon, SUM(potongan) AS jml_potongan, SUM(potongan_poin) AS potongan_poin, SUM(subtotal) AS jml_gtotal')
                                                              ->where('status_pkt', '0')
                        ...
                        $data['time2'] = strtotime($tgl);
                        $data['time1'] = strtotime('2024-05-13');
                                                
//                        if($data['time2'] >= $data['time1']){
//                            $data['sql_omset_row'] = $this->db->select('SUM(diskon) AS jml_diskon, SUM(potongan) AS jml_potongan, SUM(subtotal) AS jml_gtotal')
//                                                              ->where('DATE(tgl_bayar)', $tgl)
//                                                              ->like('tipe', $tipe, (!empty($tipe) ? 'none' : ''))
//                                                              ->like('tipe_bayar', $tipe_byr, (!empty($tipe_byr) ? 'none' : ''))
//                                                              ->like('id_poli', $poli, (!empty($poli) ? 'none' : ''))
...
//                        }else{
//                            $data['sql_omset_row'] = $this->db->select('SUM(jml_diskon) AS jml_diskon, SUM(jml_potongan) AS jml_potongan, SUM(tbl_trans_medcheck_det.potongan_poin) AS jml_potongan_poin, SUM(jml_gtotal) AS jml_gtotal')
//                                                              ->where('status_hps', '0')
//                                                              ->where('DATE(tgl_bayar)', $tgl)
//                                                              ->like('tipe', $tipe, (!empty($tipe) ? 'none' : ''))
//                                                              ->like('tipe_bayar', $tipe_byr, (!empty($tipe_byr) ? 'none' : ''))
...
//                        $data['sql_omset_row'] = $this->db->select('SUM(jml_subtotal) AS jml_subtotal, SUM(jml_diskon) AS jml_diskon, SUM(jml_potongan) AS jml_potongan, SUM(jml_gtotal) AS jml_gtotal')
//                                                          ->where('status_hps', '0')
//                                                          ->where('DATE(tgl_bayar)', $tgl)
//                                                          ->like('tipe', $tipe, (!empty($tipe) ? 'none' : ''))
//                                                          ->like('tipe_bayar', $tipe_byr, (!empty($tipe_byr) ? 'none' : ''))
...
//                                                          ->get('tbl_trans_medcheck')->row();
                        ...
                        $data['sql_omset_row'] = $this->db->select('SUM(tbl_trans_medcheck_det.diskon) AS jml_diskon, SUM(tbl_trans_medcheck_det.potongan) AS jml_potongan, SUM(tbl_trans_medcheck_det.potongan_poin) AS jml_potongan_poin, SUM(tbl_trans_medcheck_det.subtotal) AS jml_gtotal')
...
//                                                          
//                        $data['sql_omset_row'] = $this->db->select('SUM(diskon) AS jml_diskon, SUM(potongan) AS jml_potongan, SUM(potongan_poin) AS jml_potongan_poin, SUM(subtotal) AS jml_gtotal')
//                                                          ->where('status_pkt', '0')
//                                                          ->where('DATE(tgl_bayar) >=', $tgl_awal)
//                                                          ->where('DATE(tgl_bayar) <=', $tgl_akhir)
//                                                          ->like('tipe', $tipe, (!empty($tipe) ? 'none' : ''))
        ...
        if (akses::aksesLogin() == TRUE) {
            ...
            if($jml > 0){
                ...
                switch ($case){
                    case 'per_tanggal':
                        if(!empty($hal)){
                            $data['sql_penj']     = $this->db->select('id, id_medcheck, id_poli, tgl_simpan, no_rm, pasien, tipe, status, SUM(subtotal) AS jml_gtotal')
                                                         ->where('DATE(tgl_bayar)', $tgl)
                                                         ->where_in('id_item_kat', $st)
                                                         ->like('id_poli', $poli, (!empty($poli) ? 'none' : ''))
                                                         ->like('tipe', $tipe, (!empty($tipe) ? 'none' : ''))
                                                         ->group_by('id_pasien', 'tipe')
                                                         ->limit($config['per_page'], $hal)
                                                         ->get('v_medcheck_omset')->result();
                        }else{
                            $data['sql_penj']     = $this->db->select('id, id_medcheck, id_poli, tgl_simpan, no_rm, pasien, tipe, status, SUM(subtotal) AS jml_gtotal')
...
//                                ->get('tbl_trans_medcheck_det')->num_rows();
            ...
            // Config Pagination
            $config['base_url']              = base_url('laporan/data_omset_bukti.php?case='.$case.(!empty($tgl) ? '&tgl='.$tgl : '').(!empty($tgl_awal) ? '&tgl_awal='.$tgl_awal : '').(!empty($tgl_akhir) ? '&tgl_akhir='.$tgl_akhir : '').(!empty($jml_hal) ? '&jml='.$jml_hal : ''));
            $config['total_rows']            = $jml_hal;
            ...
            switch ($case){
                case 'per_tanggal':
                    if(!empty($jml)){
                        $data['sql_penj']     = $this->db->where('DATE(tgl_simpan)', $tgl_masuk)
                                                         ->limit($config['per_page'], $hal)
                                                         ->get('v_medcheck_bukti')->result();  
                    }else{
                        $data['sql_penj']     = $this->db->where('DATE(tgl_simpan)', $tgl_masuk)
                                                         ->limit($config['per_page'])
                                                         ->get('v_medcheck_bukti')->result();                        
                ...
                case 'per_rentang':
                    if(!empty($hal)){
                        $data['sql_penj']       = $this->db->where('DATE(tgl_simpan) >=', $tgl_awal)
                                                           ->where('DATE(tgl_simpan) <=', $tgl_akhir)
                                                           ->limit($config['per_page'], $hal)
                                                           ->get('v_medcheck_bukti')->result();
                    }else{
...
//
    ...
    public function data_pasien_kunj(){
        if (akses::aksesLogin() == TRUE) {
            ...
            switch ($case){                
                case 'per_tanggal':
                    $data['sql_pasien'] = $this->db->select('id, id_pasien, id_poli, tgl_simpan, poli, kode, nama, tgl_lahir, COUNT(id_pasien) AS jml_kunjungan, SUM(jml_gtotal) AS jml_gtotal')
                                                   ->where('DATE(tgl_simpan)', $tgl)
                                                   ->like('id_poli', $poli)
                                                   ->like('tipe', $tipe)
                                                   ->group_by('tipe, id_pasien')
                                                   ->order_by('COUNT(id_pasien)', 'desc')
                                                   ->get('v_medcheck_visit')->result();
                    
//                    $data['sql_pasien_row'] = $this->db->select('SUM(id_pasien) AS jml_kunjungan, SUM(jml_gtotal) AS jml_gtotal')
//                                                   ->where('DATE(tgl_simpan)', $tgl)
//                                                   ->like('id_poli', $poli)
//                                                   ->like('tipe', $tipe)
//                                                   ->group_by('tipe, id_pasien')
            ...
            switch ($case){                
                case 'per_tanggal':
                    $data['sql_pasien'] = $this->db
                                                   ->where('DATE(tgl_simpan)', $tgl)
                                                   ->like('id_pasien', general::dekrip($pasien), (!empty($pasien) ? 'none' : ''))
                                                   ->group_by('id_medcheck')
                                                   ->get('v_medcheck_visit')->result();
                    break;
    ...
    public function data_referall(){
        if (akses::aksesLogin() == TRUE) {
            ...
            if($jml > 0){
                ...
                # Config Pagination
                $config['base_url']              = base_url('laporan/data_referall.php?case='.$case.(!empty($tgl) ? '&tgl='.$tgl : '').(!empty($tgl_awal) ? '&tgl_awal='.$tgl_awal : '').(!empty($tgl_akhir) ? '&tgl_akhir='.$tgl_akhir : '').(!empty($jml) ? '&jml='.$jml : ''));
                $config['total_rows']            = $jml;
                    ...
                    case 'per_tanggal':
                        if (isset($jml)) {
                            $data['sql_referall'] = $this->db
                                                ->where('DATE(tgl_simpan)', $tgl)
                                                ->like('id_dokter', (!empty($sql_doc->id) ? $sql_doc->id : ''))
                                                ->limit($config['per_page'], $hal)
                                                ->get('v_medcheck_referall')->result();
                        } else {
                            $data['sql_referall'] = $this->db
                                                ->where('DATE(tgl_simpan)', $tgl)
                                                ->like('id_dokter', (!empty($sql_doc->id) ? $sql_doc->id : ''))
        ...
        if (akses::aksesLogin() == TRUE) {
            ...
            if(!empty($tanggal)){
                ...
                $sql = $this->db
                        ->where('DATE(tgl_simpan) >=', $tgl_awal)
                        ->where('DATE(tgl_simpan) <=', $tgl_akhir)
                        ->get('v_medcheck_referall');
                
                redirect(base_url('laporan/data_referall.php?case=per_rentang&tgl_awal='.$tgl_awal.'&tgl_akhir='.$tgl_akhir.'&jml='.$sql->num_rows()));
            }
        }else{
            $errors = $this->ion_auth->messages();
            $this->session->set_flashdata('login_toast', 'toastr.error("Authentifikasi gagal, silahkan login ulang!!");');
    ...
    public function xls_data_referall(){
        if (akses::aksesLogin() == TRUE) {
            ...
            if(!empty($tgl_awal) && !empty($tgl_akhir)){
                ...
                // Rename worksheet
                $objPHPExcel->getActiveSheet()->setTitle('Lap Referall');
                ...
                $objPHPExcel->getProperties()->setCreator("Referall Report")
                ...
                header('Content-Disposition: attachment;filename="data_referall_'.$tgl_awal.'_'.$tgl_akhir.'.xls"');
...
//                                ->like('tbl_trans_medcheck.pasien', $pasien)
    ...
    public function set_data_omset(){
        if (akses::aksesLogin() == TRUE) {
            ...
            if (!empty($tgl)) {
                ...
                $sql = $this->db
                                ->where('DATE(tgl_bayar) >=', $this->tanggalan->tgl_indo_sys($tgl_awal))
                                ->where('DATE(tgl_bayar) <=', $this->tanggalan->tgl_indo_sys($tgl_akhir))
                                ->like('tipe', $tipe, (!empty($tipe) ? 'none' : ''))
                                ->like('tipe_bayar', $pjm, (!empty($pjm) ? 'none' : ''))
                                ->like('id_poli', $poli, (!empty($poli) ? 'none' : ''))
                                ->like('metode', $plat, (!empty($plat) ? 'none' : ''))
                                ->like('pasien', $pasien)
                                ->group_by('id_pasien')
    ...
    public function set_data_omset_poli(){
        if (akses::aksesLogin() == TRUE) {
            ...
            if (!empty($tgl)){
                $sql = $this->db
                            ->where('DATE(tgl_bayar)', $tgl_masuk)
                            ->where_in('id_item_kat', $st)
                            ->like('id_poli', $poli, (!empty($poli) ? 'none' : ''))
                            ->like('tipe', $tipe, (!empty($tipe) ? 'none' : ''))
                            ->group_by('id_pasien')
    ...
    public function set_data_omset_bukti(){
        if (akses::aksesLogin() == TRUE) {
            ...
            if (!empty($tgl)) {
                ...
                redirect(base_url('laporan/data_omset_bukti.php?case=per_rentang&tgl_awal=' . $tgl_awal . '&tgl_akhir=' . $tgl_akhir . '&jml=' . $sql->num_rows()));
            }
        }else{
            $errors = $this->ion_auth->messages();
            $this->session->set_flashdata('login_toast', 'toastr.error("Authentifikasi gagal, silahkan login ulang!!");');
...
//                echo '</pre>';
    ...
    public function set_data_pasien_periksa(){
        if (akses::aksesLogin() == TRUE) {
            ...
            if (!empty($tgl)) {
                ...
                redirect(base_url('laporan/data_pemeriksaan.php?case=per_tanggal&tgl=' . $tgl_masuk.(!empty($pasien) ? '&id_pasien='.$pasien : '').'&jml=' . $sql->num_rows()));
            } elseif ($tgl_rtg) {
                $sql = $this->db->where('DATE(tgl_simpan) >=', $tgl_awal)
                                ->where('DATE(tgl_simpan) <=', $tgl_akhir)
                                ->group_by('id_medcheck')
                ...
                redirect(base_url('laporan/data_pemeriksaan.php?case=per_rentang&tgl_awal=' . $tgl_awal . '&tgl_akhir=' . $tgl_akhir.(!empty($pasien) ? '&id_pasien='.$pasien : '').'&jml=' . $sql->num_rows()));
            }
        }else{
            $errors = $this->ion_auth->messages();
            $this->session->set_flashdata('login_toast', 'toastr.error("Authentifikasi gagal, silahkan login ulang!!");');
    ...
    public function set_data_pasien_periksa_rj(){
        if (akses::aksesLogin() == TRUE) {
            ...
            if (!empty($tgl)) {
                $sql = $this->db->where('DATE(tgl_simpan)', $tgl_masuk)
                                ->get('v_medcheck_rm_rj');
                
                redirect(base_url('laporan/data_pemeriksaan_rj.php?case=per_tanggal&tgl=' . $tgl_masuk.(!empty($pasien) ? '&id_pasien='.$pasien : '').'&jml=' . $sql->num_rows()));
            } elseif ($tgl_rtg) {
                $sql = $this->db->where('DATE(tgl_simpan) >=', $tgl_awal)
                                ->where('DATE(tgl_simpan) <=', $tgl_akhir)
                                ->get('v_medcheck_rm_rj');

                redirect(base_url('laporan/data_pemeriksaan_rj.php?case=per_rentang&tgl_awal=' . $tgl_awal . '&tgl_akhir=' . $tgl_akhir.(!empty($pasien) ? '&id_pasien='.$pasien : '').'&jml=' . $sql->num_rows()));
            }
        }else{
            $errors = $this->ion_auth->messages();
            $this->session->set_flashdata('login_toast', 'toastr.error("Authentifikasi gagal, silahkan login ulang!!");');
    ...
    public function set_data_tracer(){
        if (akses::aksesLogin() == TRUE) {
            ...
            if(!empty($tgl)) {
                ...
                redirect(base_url('laporan/data_tracer.php?case=per_tanggal&tgl=' . $tgl_masuk.(!empty($tipe) ? '&tipe='.$tipe : '').(!empty($poli) ? '&poli='.$poli : '') . '&jml=' . $sql->num_rows()));
            } elseif ($tgl_rtg) {
                $sql = $this->db
                        ->where('DATE(tgl_simpan) >=', $tgl_awal)
                        ->where('DATE(tgl_simpan) <=', $tgl_akhir)
                ...
                redirect(base_url('laporan/data_tracer.php?case=per_rentang&tgl_awal=' . $tgl_awal . '&tgl_akhir=' . $tgl_akhir.(!empty($tipe) ? '&tipe='.$tipe : '').(!empty($poli) ? '&poli='.$poli : '') . '&jml=' . $sql->num_rows()));
            }
        }else{
            $errors = $this->ion_auth->messages();
            $this->session->set_flashdata('login_toast', 'toastr.error("Authentifikasi gagal, silahkan login ulang!!");');
    ...
    public function set_data_tracer_div(){
        if (akses::aksesLogin() == TRUE) {
            ...
            if(!empty($tgl)) {
                ...
                redirect(base_url('laporan/data_tracer_div.php?case=per_tanggal&tgl=' . $tgl_masuk.'&tipe='.$tipe . '&jml=' . $sql->num_rows()));
            } elseif ($tgl_rtg) {
                $sql = $this->db
                        ->where('DATE(tgl_simpan) >=', $tgl_awal)
                        ->where('DATE(tgl_simpan) <=', $tgl_akhir)
                ...
                redirect(base_url('laporan/data_tracer_div.php?case=per_rentang&tgl_awal=' . $tgl_awal . '&tgl_akhir=' . $tgl_akhir.'&tipe='.$tipe . '&jml=' . $sql->num_rows()));
            }
        }else{
            $errors = $this->ion_auth->messages();
            $this->session->set_flashdata('login_toast', 'toastr.error("Authentifikasi gagal, silahkan login ulang!!");');
...
//             Header Tabel Nota
    ...
    public function xls_data_omset_poli(){
        if (akses::aksesLogin() == TRUE) {
            ...
            switch ($case) {
                case 'per_tanggal':
                    $sql_omset      = $this->db->select('id, id_medcheck, id_item_kat, id_poli, id_dokter, tgl_simpan, no_rm, pasien, kode, item, jml, harga, subtotal, tipe, status')
                                               ->where('DATE(tgl_bayar)', $tgl)
                                               ->where_in('id_item_kat', $st)
                                               ->like('id_poli', $poli, (!empty($poli) ? 'none' : ''))
                                               ->like('tipe', $tipe, (!empty($tipe) ? 'none' : ''))
                    ...
                    $sql_omset_pas  = $this->db->select('SUM(subtotal) AS jml_gtotal')
                                               ->where('DATE(tgl_bayar)', $tgl)
                                               ->where_in('status', $st)
                                               ->like('id_poli', $poli, (!empty($poli) ? 'none' : ''))
                                               ->like('tipe', $tipe, (!empty($tipe) ? 'none' : ''))
...
//            ob_end_flush();
    ...
    public function xls_data_pasien_kunj(){
        if (akses::aksesLogin() == TRUE) {
            ...
            switch ($case){                
                case 'per_tanggal':
                    $sql_pasien = $this->db->select('id, id_pasien, id_poli, tgl_simpan, poli, kode, nama, tgl_lahir, COUNT(id_pasien) AS jml_kunjungan, SUM(jml_gtotal) AS jml_gtotal')
                                                   ->where('DATE(tgl_simpan)', $tgl)
                                                   ->like('id_poli', $poli, (!empty($poli) ? 'none' : ''))
                                                   ->like('tipe', $tipe, (!empty($tipe) ? 'none' : ''))
                                                   ->group_by('tipe, id_pasien')
        ...
        if (akses::aksesLogin() == TRUE) {
            ...
            if(!empty($sql_pasien)){
                ...
                foreach ($sql_pasien as $det){
                    $objPHPExcel->getActiveSheet()->getStyle('A'.$cell.':G'.$cell)->getAlignment()->setVertical(PHPExcel_Style_Alignment::VERTICAL_CENTER);
                    $objPHPExcel->getActiveSheet()->getStyle('A'.$cell.':B'.$cell)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
                    $objPHPExcel->getActiveSheet()->getStyle('C'.$cell.':E'.$cell)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_LEFT);
                    $objPHPExcel->getActiveSheet()->getStyle('F'.$cell)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
            ...
            // Rename worksheet
            $objPHPExcel->getActiveSheet()->setTitle('Lap Diagnosa');
            ...
            header('Content-Type: application/vnd.ms-excel');
            // header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
            header('Content-Disposition: attachment;filename="data_rekap_diagnosa_'.(isset($_GET['filename']) ? $_GET['filename'] : 'lap').'.xls"');
...
//            ob_end_flush();
    ...
    public function xls_data_tracer(){
        if (akses::aksesLogin() == TRUE) {
            ...
            // Header Tabel Nota
            $objPHPExcel->getActiveSheet()->getStyle('A1:Z5')->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
            $objPHPExcel->getActiveSheet()->getStyle('A1:Z5')->getAlignment()->setVertical(PHPExcel_Style_Alignment::VERTICAL_CENTER);
            $objPHPExcel->getActiveSheet()->getStyle('A1:Z5')->getFont()->setBold(TRUE);
            ...
            $objPHPExcel->setActiveSheetIndex(0)
                    ...
                    ->setCellValue('I4', 'Radiologi')->mergeCells('I4:M4')
                    ->setCellValue('N4', 'Farmasi')->mergeCells('N4:Q4')
                    ->setCellValue('R4', 'Rawat Inap')->mergeCells('R4:T4')
                    ->setCellValue('U4', 'Selesai')->mergeCells('U4:U5')
                    ->setCellValue('V4', 'Total')->mergeCells('V4:V5');
            ...
            $objPHPExcel->setActiveSheetIndex(0)
                    ...
                    ->setCellValue('J5', 'Kirim')
                    ->setCellValue('K5', 'Baca')
                    ->setCellValue('L5', 'Selesai')
                    ->setCellValue('M5', 'Total')
                    ->setCellValue('N5', 'Masuk')
                    ->setCellValue('O5', 'Bayar')
                    ->setCellValue('P5', 'Diterima')
                    ->setCellValue('Q5', 'Total')
                    ->setCellValue('R5', 'Masuk')
                    ->setCellValue('S5', 'Keluar')
                    ->setCellValue('T5', 'Total')
                    ->setCellValue('U5', '')
                    ->setCellValue('W5', '');
            ...
            switch ($tipe){
                default:
                    $objPHPExcel->setActiveSheetIndex(0)
                            ...
                            ->setCellValue('D4', 'Pendaftaran')->mergeCells('D4:D5')
                            ->setCellValue('E4', 'PX Dokter')->mergeCells('E4:E5')
                            ->setCellValue('F4', 'Selesai')->mergeCells('U4:U5')
                            ->setCellValue('G4', 'Total')->mergeCells('V4:V5');
                    ...
                    if(!empty($sql_tracer)){
                        ...
                        foreach ($sql_tracer as $trace){
                            ...
                            $objPHPExcel->setActiveSheetIndex(0)
                                    ...
                                    ->setCellValue('D'.$cell, $this->tanggalan->wkt_indo($trace->wkt_daftar))
                                    ->setCellValue('E'.$cell, $this->tanggalan->wkt_indo($trace->wkt_periksa))
                                    ->setCellValue('F'.$cell, $this->tanggalan->usia_wkt($trace->wkt_ranap, $trace->wkt_ranap_keluar))
                                    ->setCellValue('G'.$cell, $this->tanggalan->tgl_indo5($trace->wkt_selesai))
                                    ->setCellValue('H'.$cell, $this->tanggalan->usia_wkt($trace->wkt_daftar, $trace->wkt_selesai));
                ...
                case '1':
                    $objPHPExcel->setActiveSheetIndex(0)
                            ->setCellValue('A1', 'LAPORAN TRACER PENGUNJUNG')->mergeCells('A1:H1');
                    $objPHPExcel->setActiveSheetIndex(0)
                            ->setCellValue('A2', $pengaturan->judul)->mergeCells('A2:H2');
                    ...
                    $objPHPExcel->setActiveSheetIndex(0)
                            ...
                            ->setCellValue('D4', 'Pendaftaran')->mergeCells('D4:D5')
                            ->setCellValue('E4', 'PX Dokter')->mergeCells('E4:E5')
                            ->setCellValue('F4', 'Selesai')->mergeCells('F4:F5')
                            ->setCellValue('G4', 'Total')->mergeCells('G4:G5');
