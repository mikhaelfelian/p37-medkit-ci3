...
//                $data['jml_gtotal']   = $this->cart->total() + (($setting->jml_ppn / 100) * $this->cart->total());
    ...
    public function trans_jual_hapus() {
        if (akses::aksesLogin() == TRUE) {
            ...
            if(!empty($id)){                
                crud::update('tbl_trans_jual','id',general::dekrip($id), array('status_pjk'=>'0'));
            }
            
            redirect(base_url('pajak/'.(!empty($rute) ? $rute : 'data_penj_list.php')));
        } else {
            $errors = $this->ion_auth->messages();
            $this->session->set_flashdata('login_toast', 'toastr.error("Authentifikasi gagal, silahkan login ulang!!");');
            redirect();
    ...
    public function trans_retur_beli_det() {
        if (akses::aksesLogin() == TRUE) {
            ...
            $data['sql_ret']       = $this->db->select('DATE(tgl_simpan) as tgl_simpan, id, no_nota, id_pelanggan, jml_retur')->where('id', general::dekrip($id))->get('tbl_trans_retur_beli')->row();
    ...
    public function trans_beli_hapus() {
        if (akses::aksesLogin() == TRUE) {
            ...
            if(!empty($id)){
                ...
                $sql_mut  = $this->db->like('keterangan', 'Pembelian '.$sql_nota->no_nota)->get('tbl_m_produk_hist')->result();
...
//                    ->setTitle("Nota Penbelian " . $sql->row()->no_nota . ($sql->row()->cetak == '1' ? ' Copy Customer' : ''))
    ...
    public function input_trans_jual() {
        if (akses::aksesLogin() == TRUE) {
            /* -- Grup hak akses -- */
            ...
            $th      = $this->input->get('thn');
            $bl      = $this->input->get('bln');
            $jml     = $this->input->get('jml');
            
            $data['sql_total'] = $this->db->select('SUM(jml_gtotal) as jml_gtotal')
                                ->where('status_nota !=', '4')
                                ->where('status_pjk', '1')
                                ->like('YEAR(tgl_masuk)', $th, (!empty($bl) ? 'none' : ''))
                                ->like('MONTH(tgl_masuk)', $bl, (!empty($th) ? 'none' : ''))
            ...
            /* -- Blok Pagination -- */
//            $config['base_url']              = base_url('pajak/input_trans_jual.php?filter_nota='.$nt.'&filter_tgl='.$tg.'&filter_sales='.$sl.'&filter_status='.$sn.'&jml='.$jml);
            $config['base_url']              = base_url('pajak/input_trans_jual.php?bln='.$bl.'&thn='.$th.'&jml='.$jml_hal);
            ...
            $this->load->view('admin-lte-2/1_atas',$data);
            $this->load->view('admin-lte-2/2_header',$data);
            $this->load->view('admin-lte-2/includes/pajak/input_trans_jual',$data);
            $this->load->view('admin-lte-2/5_footer',$data);
            $this->load->view('admin-lte-2/6_bawah',$data);
        } else {
            $errors = $this->ion_auth->messages();
...
//            $jml_sql = ($id_grup->name == 'superadmin' || $id_grup->name == 'owner' || $id_grup->name == 'admin' ? $this->db->get('tbl_trans_jual')->num_rows() : $this->db->where('id_user', $id_user)->where('tgl_masuk', date('Y-m-d'))->get('tbl_trans_jual')->num_rows());
            ...
            /* -- Blok Pagination -- */
            $config['base_url']              = base_url('pajak/data_penj_list.php?filter_nota='.$nt.'&filter_tgl='.$tg.'&filter_sales='.$sl.'&filter_status='.$sn.'&jml='.$jml);
            $config['total_rows']            = $jml_hal;
            ...
            if(!empty($hal)){
                   $data['penj'] = $this->db->select('id, id_app, no_nota, kode_nota_dpn, kode_nota_blk, kode_nota_dpn, kode_nota_blk, DATE(tgl_masuk) as tgl_masuk, DATE(tgl_bayar) as tgl_bayar, DATE(tgl_keluar) as tgl_keluar, jml_total, jml_gtotal, ppn, jml_ppn, id_user, id_sales, id_pelanggan, status_nota, status_bayar, status_grosir')
                           ...
                           ->like('id_pelanggan', $cs)
                           ->like('id_sales', $sl)
                           ->like('id_user', ($id_grup->name == 'superadmin' || $id_grup->name == 'owner' || $id_grup->name == 'owner2' || $id_grup->name == 'admin' ? '' : $id_user))
                           ->like('tgl_masuk', ($id_grup->name == 'superadmin' || $id_grup->name == 'owner' || $id_grup->name == 'owner2' || $id_grup->name == 'admin' ? '' : date('Y-m-d')))
            ...
            $this->load->view('admin-lte-2/1_atas',$data);
            $this->load->view('admin-lte-2/2_header',$data);
            $this->load->view('admin-lte-2/includes/pajak/data_penjualan_list',$data);
            $this->load->view('admin-lte-2/5_footer',$data);
            $this->load->view('admin-lte-2/6_bawah',$data);
        } else {
            $errors = $this->ion_auth->messages();
...
//                           ->or_like('kode_nota_blk', $nt, 'after')
    ...
    public function data_pemb_jual_list() {
        if (akses::aksesLogin() == TRUE) {
            ...
            /* -- jml halaman pada list -- */
            if(akses::hakSA() == TRUE OR akses::hakAdmin() == TRUE){
                $jml_hal = (!empty($jml) ? $jml  : $this->db->where('status_bayar !=', '1')->where('status_pjk', '1')->where('id_user', $id_user)->get('tbl_trans_jual')->num_rows());
            }else{
                /* -- Hitung jumlah halaman -- */
                $jml_hal = (!empty($jml) ? $jml  : $this->db->where('status_bayar !=', '1')->where('status_pjk', '1')->where('id_user', $id_user)->get('tbl_trans_jual')->num_rows());
            }
            ...
            /* -- Blok Pagination -- */
            $config['base_url']              = base_url('pajak/data_pemb_jual_list.php?filter_nota='.$tg.'&filter_tgl='.$tg.'&filter_sales='.$sl.'&filter_status='.$sn.'&jml='.$jml);
            $config['total_rows']            = $jml_hal;
            ...
            if(!empty($hal)){
                   $data['penj'] = $this->db->select('id, id_app, no_nota, kode_nota_dpn, kode_nota_blk, DATE(tgl_masuk) as tgl_masuk, DATE(tgl_bayar) as tgl_bayar, DATE(tgl_keluar) as tgl_keluar, jml_total, jml_diskon, jml_subtotal, ppn, jml_ppn, jml_gtotal, jml_bayar, jml_kurang, id_user, id_pelanggan, status_nota, status_bayar')
                           ...
                           ->like('id_user', ($id_grup->name == 'superadmin' || $id_grup->name == 'owner' || $id_grup->name == 'owner2' ? '' : $id_user))
                           ->order_by('id','desc')
                           ->get('tbl_trans_jual')->result();
            }else{
                   $data['penj'] = $this->db->select('id, id_app, no_nota, kode_nota_dpn, kode_nota_blk, DATE(tgl_masuk) as tgl_masuk, DATE(tgl_bayar) as tgl_bayar, DATE(tgl_keluar) as tgl_keluar, jml_total, jml_diskon, jml_subtotal, ppn, jml_ppn, jml_gtotal, jml_bayar, jml_kurang, id_user, id_pelanggan, status_nota, status_bayar')
            ...
            $this->load->view('admin-lte-2/1_atas',$data);
            $this->load->view('admin-lte-2/2_header',$data);
            $this->load->view('admin-lte-2/includes/pajak/data_pembayaran_list',$data);
            $this->load->view('admin-lte-2/5_footer',$data);
            $this->load->view('admin-lte-2/6_bawah',$data);
        } else {
            $errors = $this->ion_auth->messages();
    ...
    public function data_retur_jual_list() {
        if (akses::aksesLogin() == TRUE) {
            ...
            /* -- jml halaman pada list -- */
            $jml_hal = (!empty($jml) ? $jml  : $this->db->join('tbl_trans_jual', 'tbl_trans_jual.id=tbl_trans_retur_jual.id_penjualan')->where('tbl_trans_jual.status_pjk', '1')->like('tbl_trans_jual.no_nota', $nt)->like('DATE(tbl_trans_retur_jual.tgl_simpan)', $tg)->like('tbl_trans_retur_jual.status_retur', $sn)->get('tbl_trans_retur_jual')->num_rows());

            /* -- Form Error -- */
            $data['hasError']                = $this->session->flashdata('form_error');

            /* -- Blok Pagination -- */
            $config['base_url']              = base_url('pajak/data_retur_jual_list.php?filter_nota='.$tg.'&filter_tgl='.$tg.'&filter_sales='.$sl.'&filter_status='.$sn.'&jml='.$jml);
            $config['total_rows']            = $jml_hal;
            ...
            if(!empty($hal)){
                   $data['retur'] = $this->db->select('DATE(tbl_trans_retur_jual.tgl_simpan) as tgl_simpan, tbl_trans_retur_jual.id, tbl_trans_retur_jual.no_retur, tbl_trans_retur_jual.no_nota, tbl_trans_retur_jual.status_retur, tbl_trans_retur_jual.jml_retur, tbl_m_pelanggan.nama')
                                         ...
                                         ->where('tbl_trans_jual.status_pjk', '1')
                                         ->limit($config['per_page'],$hal)
                                         ->like('tbl_trans_jual.id_user', ($id_grup->name == 'superadmin' || $id_grup->name == 'owner' || $id_grup->name == 'owner2' ? '' : $id_user))
            ...
            $this->load->view('admin-lte-2/1_atas',$data);
            $this->load->view('admin-lte-2/2_header',$data);
            $this->load->view('admin-lte-2/includes/pajak/data_retur_jual_list',$data);
            $this->load->view('admin-lte-2/5_footer',$data);
            $this->load->view('admin-lte-2/6_bawah',$data);
        } else {
            $errors = $this->ion_auth->messages();
        ...
        if (akses::aksesLogin() == TRUE) {
            ...
            if(!empty($hal)){
                   $data['retur'] = $this->db->select('DATE(tbl_trans_retur_beli.tgl_simpan) as tgl_simpan, tbl_trans_retur_beli.id, tbl_trans_retur_beli.no_nota, tbl_trans_retur_beli.id_pembelian, tbl_trans_retur_beli.status_retur, tbl_trans_retur_beli.jml_retur, tbl_m_supplier.nama')
                             ->join('tbl_trans_beli', 'tbl_trans_beli.no_nota=tbl_trans_retur_beli.no_nota')
                             ->join('tbl_m_supplier', 'tbl_m_supplier.id=tbl_trans_beli.id_supplier')
//                           ->limit($config['per_page'],$hal)
//                           ->like('no_nota', $nt)
//                           ->like('DATE(tgl_masuk)', $tg)
...
//            $jml_hal = (!empty($jml) ? $jml : $jml_sql); // (!empty($jml) ? $jml  : ($id_grup->name == 'superadmin' OR $id_grup->name == 'owner' ? $jml_kueri : $this->db->where('id_user', $id_user)->get('tbl_trans_jual')->num_rows()));
            ...
            /* -- Blok Pagination -- */
            $config['base_url']              = base_url('pajak/input_retur_jual.php?filter_nota='.$nt.'&filter_tgl='.$tg.'&filter_sales='.$sl.'&filter_status='.$sn.'&jml='.$jml);
            $config['total_rows']            = $jml_hal;
            ...
            $this->load->view('admin-lte-2/1_atas',$data);
            $this->load->view('admin-lte-2/2_header',$data);
            $this->load->view('admin-lte-2/includes/pajak/input_retur_jual',$data);
            $this->load->view('admin-lte-2/5_footer',$data);
            $this->load->view('admin-lte-2/6_bawah',$data);
        } else {
            $errors = $this->ion_auth->messages();
...
//                    $this->fpdf->Cell(3, .5, ucwords($sales->nama), 1, 0, 'L', $fill);
    ...
    public function set_nota_jual_simpan() {
        if (akses::aksesLogin() == TRUE) {
            ...
            if (empty($no_nota)) {
                ...
                redirect(base_url('pajak/input_trans_jual.php'));
            } else {
                $sql_nota   = $this->db->where('id', general::dekrip($no_nota))->get('tbl_trans_jual');
                $nota       = $sql_nota->row();
                ...
                if($sql_nota->num_rows() == '1'){
                    crud::update('tbl_trans_jual', 'id', general::dekrip($no_nota), $data);
                }else{
                    $msg_err = 'Data tidak ditemukan!!';
                }
                redirect(base_url('pajak/input_trans_jual.php?halaman='.$hal.(!empty($bln) ? '&bln='.$bln.'&thn='.$thn : '').(!empty($msg_err) ? '&msg='.$msg_err : '')));
            }
        } else {
            $errors = $this->ion_auth->messages();
            $this->session->set_flashdata('login_toast', 'toastr.error("Authentifikasi gagal, silahkan login ulang!!");');
    ...
    public function set_nota_jual_simpan2() {
        if (akses::aksesLogin() == TRUE) {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                foreach ($_POST['input'] as $kunci => $pst){
                    crud::update('tbl_trans_jual', 'id', $kunci, array('status_pjk'=>'1'));
                }
                
                redirect(base_url('pajak/input_trans_jual.php?bln='.$bln.'&thn='.$thn.(!empty($hal) ? '&halaman='.$hal : '')));
            }
    ...
    public function set_nota_jual_umum() {
        if (akses::aksesLogin() == TRUE) {
            ...
            $data = array(
                ...
                'id_pelanggan' => (isset($plgn) ? $plgn : 1),
                'id_kategori'  => 0,
        ...
        if (akses::aksesLogin() == TRUE) {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                foreach ($sql_beli_det as $sql_det){
                    ...
                    $data_hist = array(
                        'tgl_modif'  => date('Y-m-d H:i:s'),
                        'keterangan' => 'Pembelian '.anchor(base_url('transaksi/trans_beli_det.php?id='.general::enkrip($sql_beli->id)), $sql_beli->no_nota, 'target="_blank"')
                    );
...
//                }else{
                    ...
                    $data_penj = array(
                        'satuan'        => $edit_sat,
                        'keterangan'    => ($edit_jml != $sql_satuan->jml ? ' ('.$sql_satuan->jml * $edit_jml.' '.$sql_satuan2->satuanTerkecil.')' : ''),
                        'harga'         => ceil((float)$edit_hrg),
                        'jml'           => (float)$edit_jml,
...
//            } else {
                ...
                foreach ($sql_penj_det as $beli){
                    $hrg_ppn        = ($ppn > 0 ? ($ppn / 100) * $beli->harga : 0);
                    $hrg_ppn_beli   = $beli->harga + $hrg_ppn;
                    
                    crud::update('tbl_m_produk', 'id', $beli->id_produk, array('harga_beli' => $hrg_ppn_beli));
                }
...
//                           ->get('tbl_trans_jual')->num_rows();
            ...
            redirect(base_url('pajak/data_penj_list.php?'.(!empty($no_nota) ? 'filter_nota='. substr($no_nota, 1) : '').(!empty($t_nota) ? '&filter_tgl='.$tgl_nota : '').(!empty($t_tempo) ? '&filter_tgl_tempo='.$tgl_tempo : '').(!empty($customer) ? '&filter_cust='.$customer : '').(!empty($sales) ? '&filter_sales='.$sales : '').(!empty($t_bayar) ? '&filter_tgl_bayar='.$t_bayar : '')));
        } else {
            $errors = $this->ion_auth->messages();
            $this->session->set_flashdata('login_toast', 'toastr.error("Authentifikasi gagal, silahkan login ulang!!");');
            redirect();
    ...
    public function set_cari_penj2() {
        if (akses::aksesLogin() == TRUE) {
            $bln = $this->input->post('bulan');
            $thn = $this->input->post('tahun');
            
            redirect(base_url('pajak/input_trans_jual.php'.(!empty($bln) ? '?bln='.$bln.'&thn='.$thn : '')));
        } else {
            $errors = $this->ion_auth->messages();
            $this->session->set_flashdata('login_toast', 'toastr.error("Authentifikasi gagal, silahkan login ulang!!");');
            redirect();
    ...
    public function set_cari_penj_retur() {
        if (akses::aksesLogin() == TRUE) {
            ...
            redirect(base_url('transaksi/data_retur_jual_list.php?'.(!empty($no_nota) ? 'filter_nota='. substr($no_nota, 1) : '').(!empty($t_nota) ? '&filter_tgl='.$tgl_nota : '').(!empty($t_tempo) ? '&filter_tgl_tempo='.$tgl_tempo : '').(!empty($customer) ? '&filter_cust='.$customer : '').(!empty($sales) ? '&filter_sales='.$sales : '').'&jml='.$sql));
        } else {
            $errors = $this->ion_auth->messages();
            $this->session->set_flashdata('login_toast', 'toastr.error("Authentifikasi gagal, silahkan login ulang!!");');
            redirect();
    ...
    public function set_cari_penj_retur_input() {
        if (akses::aksesLogin() == TRUE) {
            ...
            redirect(base_url('pajak/input_retur_jual.php?'.(!empty($no_nota) ? 'filter_nota='. substr($no_nota, 1) : '').(!empty($t_nota) ? '&filter_tgl='.$tgl_nota : '').(!empty($t_tempo) ? '&filter_tgl_tempo='.$tgl_tempo : '').(!empty($customer) ? '&filter_cust='.$customer : '').(!empty($sales) ? '&filter_sales='.$sales : '')));
        } else {
            $errors = $this->ion_auth->messages();
            $this->session->set_flashdata('login_toast', 'toastr.error("Authentifikasi gagal, silahkan login ulang!!");');
            redirect();
...
//                           ->or_like('kode_nota_blk', $nt, 'after')
    ...
    public function set_cari_pemb() {
        if (akses::aksesLogin() == TRUE) {
            ...
            redirect(base_url('pajak/data_pembelian_list'.(!empty($rute) ? '_tempo' : '').'.php?'.(!empty($rute) ? 'route=tempo&' : '').(!empty($no_nota) ? 'filter_nota='.$no_nota : '').(!empty($lokasi) ? '&filter_lokasi='.$lokasi : '').(!empty($tgl_trans) ? '&filter_tgl='.$tgl[2] . '-' . $tgl[0] . '-' . $tgl[1] : '').(!empty($tgl_tempo) ? '&filter_tgl_tempo='.$this->tanggalan->tgl_indo_sys($tgl_tempo) : '').(!empty($supplier) ? '&filter_supplier='.$supplier : '').(!empty($tgl_bayar) ? '&filter_tgl_bayar='.$this->tanggalan->tgl_indo_sys($tgl_bayar) : '').(isset($sb) && !empty($sb) ? '&filter_bayar='.$sb : '')));
        } else {
            $errors = $this->ion_auth->messages();
            $this->session->set_flashdata('login_toast', 'toastr.error("Authentifikasi gagal, silahkan login ulang!!");');
            redirect();
